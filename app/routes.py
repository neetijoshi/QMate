...
def calculate_wait_time(queue_id, token_number):
    ...
    avg_time = sum(c.avg_service_time for c in counters) / len(counters)

    return round(people_ahead * avg_time)
...
def set_queue_settings():
    try:
        ...
        if active_tokens > 0:
            return jsonify({
                "error": "Cannot change queue while tokens are active"
            }), 400

        # ✅ DEACTIVATE OLD COUNTERS (NO DELETE)
        old_counters = Counter.query.filter_by(
            queue_id=queue_id,
            is_active=True
        ).all()
        ...
        db.session.commit()
        return jsonify({"message": "Queue optimization updated"}), 200

    except Exception as e:
        db.session.rollback()
        print("SET QUEUE SETTINGS ERROR:", e)
        return jsonify({"error": "Server error"}), 500

@main.route("/admin/pause_queue", methods=["POST"])
def pause_queue():
    if session.get("role") != "admin":
        return jsonify({"error": "Unauthorized"}), 403
    ...
    return jsonify({
        "message": f"{queue.name} queue has been paused"
    })

@main.route("/public/paused_queues")
def paused_queues():
    from .models import Queue

    paused = Queue.query.filter_by(status="PAUSED").all()
...
def next_token():
    ...
    # 🔄 AUTO-UNPAUSE QUEUE WHEN NEXT IS CLICKED
